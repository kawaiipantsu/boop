...
func run(args []string, stdout, stderr io.Writer) error {
	...
	if len(args) > 0 {
		...
		case "status":
			return runStatus(context.Background(), options{}, stdout, stderr)
		}
	}
...
func parse(args []string, stderr io.Writer) (options, error) {
	...
	fs.BoolVar(&opts.showStatus, "status", false, "print runtime status and provider health")
...
usage:
  ...
  boop status                       print runtime status and provider health
